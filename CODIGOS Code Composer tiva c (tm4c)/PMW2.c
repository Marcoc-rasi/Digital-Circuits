...
int main(void)
{
    ...
    SYSCTL_RCGCGPIO_R = SYSCTL_RCGCGPIO_R12 |SYSCTL_RCGCGPIO_R3 |SYSCTL_RCGCGPIO_R4; //HABILITA PORTN Y PORTD y PORTE

            SYSCTL_RCGCTIMER_R |= 0X08 | 0X04; //HABILITA TIMER 3
            ...
            //CORRESPONDIENTE A PD4 T PD5
                  // habilita el bit 4 como digital
                  // configura como salida
                  //

                 GPIO_PORTD_AHB_DEN_R |= 0x30; //BIT 4 DIGITAL
                  GPIO_PORTD_AHB_DIR_R |= 0x30; //bit 4 SALIDA
                  GPIO_PORTD_AHB_DATA_R = 0x00; // SALIDA A 0
                  GPIO_PORTD_AHB_AFSEL_R = 0x30; //FUNCION ALTERNA EN BIT 4 0001 0000
    ...
    //para el servomotor el valor de ilr debe ser 320,000 hex=4e200
            ...
            TIMER3_TAILR_R= 0xfff; // VALOR DE RECARGA 1 KHz
            TIMER3_TAMATCHR_R =0x3ff; // 50 %
            TIMER3_TBILR_R= 0xfff; // VALOR DE RECARGA 1 KHz
...
//habilitar reloj para los puertos utilizados
 ...
 //inicializar PE.3 para que sea AIN0
     ...
     GPIO_PORTE_AHB_AMSEL_R |=0X18;  //habilita modulo analogico  0000 1000   0001 0000
...
// inicializar adc
    ...
    ADC0_SSEMUX2_R  = 0;         //obtener entrada del primer grupo de 16 canales (0 al 15) p1146
    ADC0_SSMUX2_R  = 0;         //obtener entrada del canal 0 p1141                                 0000 1001
    ...
    ADC0_ACTSS_R   |= 0X0C;        //habilitar secuenciador 3 del ADC0  p1077
    ...
    ADC0_ISC_R   = 8;                      //limpiar bandera
        ...
        //ADC0_SSEMUX3_R  = 0;         //obtener entrada del primer grupo de 16 canales (0 al 15) p1146
        //ADC0_SSMUX3_R  = 0;
        ADC0_PSSI_R   |= 4;               //EMPEZAR SECUENCIA DE CONVERSION 3 p1103
        while((ADC0_RIS_R & 4 == 0));  //espera por conversión completa  p1079
        ValorADC  = ADC0_SSFIFO2_R ;       //leer el resultado de la conversion p1118

        TIMER3_TBMATCHR_R= ValorADC;


       ADC0_ISC_R   = 4;                      //limpiar bandera
               }
